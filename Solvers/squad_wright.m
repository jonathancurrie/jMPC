function [z,exitflag,iter,lam,t] = squad_wright(H,f,A,b,maxiter,tol,verbose,z0,lam0,t0)
% Solve quadratic programming problem using Wright's (1997) Method [Single Precision]
...
%Default Values
sigma = single(0.1); ascale = single(1); inftol = tol*10;
exitflag = 1; cholfail = 0;
mu = t'*lam / mc; At = A';
mr2_1 = single(100); mr2 = single(10);
%Initial Residuals
r1 = -H*z - At*lam - f;
r2 = -A*z + b;
%Linsolve options
...
if(verbose)
    fprintf('-----------------------------------------------\n');
    fprintf('QuadWright QP Solver [MATLAB Single Version]\n');
    switch(wmode)
...
for iter = 1:maxiter
    ...
    if(iter > 6 && mr2/pmax > single(tol/10))
    ...
    sigma = min((mu/muold)^3,single(0.99999));
end
